        ...
        try {
          const result = await step.run(`generate-report-${user.id}`, async () => {
            ...
            const stats = await Promise.race([
              getMonthlyStats(user.id, lastMonth),
              new Promise((_, reject) => 
                setTimeout(() => reject(new Error("Stats generation timed out")), 120000)
              )
            ]);
            ...
            // Generate AI insights with timeout and retry
            const insights = await step.run("generate-insights", async () => {
              try {
                return await Promise.race([
                  generateFinancialInsights(stats, monthName),
                  new Promise((_, reject) => 
                    setTimeout(() => reject(new Error("Insights generation timed out")), 120000)
                  )
                ]);
              } catch (error) {
                console.error("Error generating insights:", error);
            ...
            await step.run("send-email", async () => {
              try {
                await Promise.race([
                  ...
                  new Promise((_, reject) => 
                    setTimeout(() => reject(new Error("Email sending timed out")), 180000)
                  )
                ]);
                return { success: true };
              } catch (error) {
